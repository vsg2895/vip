    ...
    <div class="row no-gutters">
        ...
        <div class="col-lg-8">
            ...
            @if(\Request::session()->has('add_post_input_'.$input_item->id) && \Request::session()->get('add_post_input_'.$input_item->id) != NULL)
                <input type="date" name="input_{{ $input_item->id }}" form="addPostForm" minlength="1" maxlength="255" class="input-default w-100 p-2" placeholder="@if(isset($input_item->has_placeholder) && $input_item->has_placeholder == 1) {{ $input_item->{'title_'.app()->getLocale()} }}  @endif" value="{{ \Request::session()->get('add_post_input_'.$input_item->id) }}">
            @else
