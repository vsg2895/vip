    ...
    <div class="detail-sldier mt-2">
        ...
        <div class="slider slider-for">
            @if(isset($post->img) && $post->img != NULL)
                <div>
                    <img src="{{ asset('assets/img/items').'/'.$post->img }}" data-index="0"
                         class="responsive slider-large rounded" title="{{ $post->title }}" alt="{{ $post->title }}">
                </div>
            @endif

            @foreach($post->image as $key => $image)
                <div>
                    <img src="{{ asset('assets/img/items').'/'.$image->img }}" data-index="{{ ++$key }}"
                         class="responsive slider-large rounded" title="{{ $post->title }}" alt="{{ $post->title }}">
                </div>
            @endforeach
        </div>
        ...
        <div class="slider slider-nav thumbnails">
            @if(isset($post->img) && $post->img != NULL)
                <div>
                    <img src="{{ asset('assets/img/items').'/'.$post->img }}" data-index="0"
                         class="w-100 h-100 responsive rounded" title="{{ $post->title }}" alt="{{ $post->title }}">
                </div>
            @endif

            @foreach($post->image as $key_2 => $image)
                <div>
                    <img src="{{ asset('assets/img/items').'/'.$image->img }}" data-index="{{ ++$key_2 }}"
                         class="w-100 h-100 responsive rounded" title="{{ $post->title }}" alt="{{ $post->title }}">
                </div>
            @endforeach
        </div>
        <div class="detail-like-action like-button-container">
            <!-- Wishlist Action -->
        @auth <!-- Add to wishlist -->
            <div class="like-action">
            @if(!isset($post->wishlist) || $post->wishlist == NULL)
                <!-- Not Liked -->
                    <svg class="float-right wishlist-action not-liked"
                         data-url="{{ route('post-wishlist-action', ['locale' => app()->getLocale(), 'post_id' => $post->id]) }}"
                         xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 22.507 23.154">
            ...
            @else
                <!-- Liked -->
                    <svg class="wishlist-action float-right"
                         data-url="{{ route('post-wishlist-action', ['locale' => app()->getLocale(), 'post_id' => $post->id]) }}"
                         xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24.523 24.557">
                        <g id="like_" data-name="like " transform="translate(0 0.034)">
                            <path id="Path_563" data-name="Path 563"
                                  d="M56.364,265.475v10.4a.936.936,0,0,1-.935.935h-3.9v.006a.936.936,0,0,1-.935-.935V265.475a.936.936,0,0,1,.935-.935h3.9A.936.936,0,0,1,56.364,265.475Z"
...
@else
    ...
    @if(isset($post->img) && $post->img != NULL)
        ...
        <div class="w-100 mt-2 single-img">
            <img src="{{ asset('assets/img/items').'/'.$post->img }}" data-index="0"
                 class="w-100 slider-large responsive" title="{{ $post->title }}" alt="{{ $post->title }}">

            <div class="like-button-container detail-like-action">
                <!-- Wishlist Action -->
            @auth <!-- Add to wishlist -->
                <div class="like-action">
                @if(!isset($post->wishlist) || $post->wishlist == NULL)
...
<div id="box">
    ...
    <!-- Arrows -->
    <span class="arrows no-click" id="left"><i class="fa text-light fa-3x fa-angle-left"></i></span>
    <span class="arrows no-click" id="right"><i class="text-light fa-3x fa fa-angle-right"></i></span>

    <!-- Title -->
    <div class="item-title no-click">{{ $post->title }}</div>

    <!-- Close Button -->
    <span class="close"><i class="fa fa-times fa-2x text-light"></i></span>
</div>
