<div class="row default-row mt-xl-2 mt-lg-2 mt-md-2 mt-3 justify-content-center">
    ...
    @if(isset($locations) && count($locations) > 0)
        ...
        <div class="form-group col-md-3 col-sm-6 col-6">
            ...
            <select name="estate_type" form="filteringForm" class="input-default w-100 p-2">
            {{-- adds controllers category and filtr functions new session filter_post_estate_type--}}
            <!-- Default value -->
                <option value="default"
                        @if(\Request::session()->has('filter_post_estate_type') && \Request::session()->get('filter_post_estate_type') == 'default') selected @endif>{{ translating('post_estate_type') }}</option>
                <!-- Other Values -->
                <option value="0"
                        @if(\Request::session()->has('filter_post_estate_type') && \Request::session()->get('filter_post_estate_type') == '0')  selected @endif>{{ translating('filter_estate_rent') }}</option>
                <option value="1"
                        @if(\Request::session()->has('filter_post_estate_type') && \Request::session()->get('filter_post_estate_type') == '1')  selected @endif>{{ translating('filter_daily_rent') }}</option>
                <option value="2"
                        @if(\Request::session()->has('filter_post_estate_type') && \Request::session()->get('filter_post_estate_type') == '2')  selected @endif>{{ translating('filter_estate_sale') }}</option>
                <option value="3"
                        @if(\Request::session()->has('filter_post_estate_type') && \Request::session()->get('filter_post_estate_type') == '3')  selected @endif>{{ translating('filter_estate_change') }}</option>
            </select>
