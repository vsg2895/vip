...
<div class="row mb-5 item-card-row post-activation-type-{{ $post->active }}" data-item-id="{{ $post->id }}"
     data-item-update-date="{{ date_default_format($post->updated_at) }}" data-item-updates="{{ $post->updates }}">
    <!-- Image -->
    ...
    <div class="col-lg-9 col-md-8 mt-0 mt-sm-2 col-sm-12">
        ...
        <div class="row no-gutters w-100">
            <!-- URL -->
            <a class="text-dark" href="{{ route('items', ['locale' => app()->getLocale(), 'id' => $post->id]) }}">
        ...
        <!-- Price -->
        <div class="row no-gutters w-100">
            <span class="font-weight-bold">{{ mb_substr($post->price.' '.$post->currency['simbol'], 0, 20) }}</span>
        </div>
        ...
        <!-- Breadcrumbs -->
        <div class="row no-gutters breadcrumbs-for-card w-100 mt-2">
            <!-- Check data -->
        @foreach(array_reverse(getPostBreadcrumbs($post->category_id)) as $key => $breadcrumbs_item)
            <!-- Last item in loop -->
            @if($loop->last)
                <!-- URL -->
                    <a class="text-secondary"
                       href="{{ route('category', ['locale' => app()->getLocale(), 'id' => $breadcrumbs_item->id]) }}">&nbsp;{{ $breadcrumbs_item->title }}</a>
            @else
                <!-- URL -->
                    <a class="text-secondary"
                       href="{{ route('category', ['locale' => app()->getLocale(), 'id' => $breadcrumbs_item->id]) }}">
                        {{ $breadcrumbs_item->title }}&nbsp;
                        <!-- Icon -->
                        <svg xmlns="http://www.w3.org/2000/svg" width="4.559" height="9.014" viewBox="0 0 4.559 9.014">
                            <g id="left-arrow" transform="translate(4.559 9.014) rotate(180)">
                                <g id="Group_9" data-name="Group 9" transform="translate(0)">
                                    <path id="Path_5" data-name="Path 5"
                                          d="M4.457,8.413.807,4.756a.353.353,0,0,1,0-.5L4.457.6a.352.352,0,0,0-.5-.5L.309,3.76a1.057,1.057,0,0,0,0,1.494L3.958,8.911a.352.352,0,0,0,.5-.5Z"
                                          fill="#6e6e6e"/>
        ...
        <!-- Actions -->
        <div class="row no-gutters w-100 bottom-row-with-like">
            <!-- Actions -->
            ...
            @else <!-- Log in to add to wishlist -->
                <svg data-toggle="modal" data-target="#logInToAddWishlistCenter" class="float-right"
                     xmlns="http://www.w3.org/2000/svg" width="22.507" height="23.154" viewBox="0 0 22.507 23.154">
                    <g id="like_" data-name="like " transform="translate(0 0.034)">
                        <path id="Path_563" data-name="Path 563"
                              d="M56.364,265.475v10.4a.936.936,0,0,1-.935.935h-3.9v.006a.936.936,0,0,1-.935-.935V265.475a.936.936,0,0,1,.935-.935h3.9A.936.936,0,0,1,56.364,265.475Z"
                              transform="translate(-50.59 -252.544)" fill="#555555"/>
