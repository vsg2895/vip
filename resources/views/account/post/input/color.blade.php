    ...
    <div class="row no-gutters">
        ...
        <div class="col-lg-8">    
            ...
            @if(isset($post->option[$input_key]['value']) && $post->option[$input_key]['value'] != NULL)
                <!-- Input -->
                <input type="color" name="input_{{ $input_item->id }}" form="addPostForm" minlength="1" maxlength="255" class="input-default w-100 p-2" placeholder="@if(isset($input_item->has_placeholder) && $input_item->has_placeholder == 1) {{ $input_item->{'title_'.app()->getLocale()} }}  @endif" value="{{ $post->option[$input_key]['value'] }}">
            @else
                <!-- Input -->
