 ...
 <div class="row mt-4">
    ...
    <div class="col-md-4 col-sm-6">
        <div class="row no-gutters">
            <div class="col-xl-3 col-lg-5 col-sm-4 col-3">
                @if(isset($countries) && count($countries) > 0)
                    <select style="-webkit-appearance: none;" form="changeUserPhonesForm" name="country_id" class="w-100 no-rounded input-default p-2" required>
                        @foreach($countries as $country)
                            @if(isset(Auth::user()->country_id) && Auth::user()->country_id != NULL)
                                <option value="{{ $country->id }}" @if($country->code == $user['country']['code']) selected @endif>{{ '+ '.$country->code.' '.$country->{'title_'.app()->getLocale()} }}</option>
                            @else
                                <option value="{{ $country->id }}" @if($country->id == 121) selected @endif>{{ '+ '.$country->code.' '.$country->{'title_'.app()->getLocale()} }}</option>
                            @endif
                        @endforeach
                    </select>
                @else
            ...
            <div class="col-xl-9 col-lg-7 col-sm-8 col-9">
                <input value="@if(isset($user['phone'])) {{ $user['phone'] }} @endif" required oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" form="changeUserPhonesForm" type="text" class="input-default w-100 p-2 no-rounded" min="1" max="255" placeholder="{{ translating('placeholder-primary-phone-number') }}" name='phone'>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="row mt-4">
        ...
        <div class="col-md-4 col-sm-6">
            <div class="row no-gutters">
                <div class="col-xl-3 col-lg-5 col-sm-4 col-3">
                    @if(isset($countries) && count($countries) > 0)
                        <select style="-webkit-appearance: none;" form="changeUserPhonesForm" name="phone_country_id" class="w-100 no-rounded input-default p-2" required>
                            @foreach($countries as $country)
                                @if(isset($user['phone_number']['phone_country']) && $user['phone_number']['phone_country'] != NULL)
                                    <option value="{{ $country->id }}" @if($country->id == $user['phone_number']['phone_country_id']) selected @endif>{{ '+ '.$country->code.' '.$country->{'title_'.app()->getLocale()} }}</option>
                                @else
                                    <option value="{{ $country->id }}" @if($country->id == 121) selected @endif>{{ '+ '.$country->code.' '.$country->{'title_'.app()->getLocale()} }}</option>
                                @endif
                            @endforeach
                ...
                <div class="col-xl-9 col-lg-7 col-sm-8 col-9">
                    <input value="@if(isset($user['phone_number']['phone_number'])) {{ $user['phone_number']['phone_number'] }} @endif" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" form="changeUserPhonesForm" type="text" class="input-default w-100 p-2 no-rounded" min="1" max="255" placeholder="{{ translating('placeholder-phone-number') }}" name='phone_number'>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="row mt-4">
            ...
            <div class="col-md-4 col-sm-6">
                <div class="row no-gutters">
                    <div class="col-xl-3 col-lg-5 col-sm-4 col-3">
                        @if(isset($countries) && count($countries) > 0)
                            <select style="-webkit-appearance: none;" form="changeUserPhonesForm" name="viber_country_id" class="w-100 no-rounded input-default p-2" required>
                                @foreach($countries as $country)
                                    @if(isset($user['phone_number']['viber_country_id']) && $user['phone_number']['viber_country_id'] != NULL)
                                        <option value="{{ $country->id }}" @if($country->id == $user['phone_number']['viber_country_id']) selected @endif>{{ '+ '.$country->code.' '.$country->{'title_'.app()->getLocale()} }}</option>
                                    @else
                                        <option value="{{ $country->id }}" @if($country->id == 121) selected @endif>{{ '+ '.$country->code.' '.$country->{'title_'.app()->getLocale()} }}</option>
                                    @endif
                                @endforeach
                    ...
                    <div class="col-xl-9 col-lg-7 col-sm-8 col-9">
                        <input value="@if(isset($user['phone_number']['viber'])) {{ $user['phone_number']['viber'] }} @endif" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" form="changeUserPhonesForm" type="text" class="input-default w-100 p-2 no-rounded" min="1" max="255" placeholder="{{ translating('placeholder-viber-number') }}" name='viber'>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row mt-4">
            ...
            <div class="col-md-4 col-sm-6">
                <div class="row no-gutters">
                    <div class="col-xl-3 col-lg-5 col-sm-4 col-3">
                        @if(isset($countries) && count($countries) > 0)
                            <select style="-webkit-appearance: none;" form="changeUserPhonesForm" name="whatsapp_country_id" class="w-100 no-rounded input-default p-2" required>
                                @foreach($countries as $country)
                                    @if(isset($user['phone_number']['whatsapp_country_id']) && $user['phone_number']['whatsapp_country_id'] != NULL)
                                        <option value="{{ $country->id }}" @if($country->id == $user['phone_number']['whatsapp_country_id']) selected @endif>{{ '+ '.$country->code.' '.$country->{'title_'.app()->getLocale()} }}</option>
                                    @else
                                        <option value="{{ $country->id }}" @if($country->id == 121) selected @endif>{{ '+ '.$country->code.' '.$country->{'title_'.app()->getLocale()} }}</option>
                                    @endif
                                @endforeach
                    ...
                    <div class="col-xl-9 col-lg-7 col-sm-8 col-9">
                        <input value="@if(isset($user['phone_number']['whatsapp'])) {{ $user['phone_number']['whatsapp'] }} @endif" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" form="changeUserPhonesForm" type="text" class="input-default w-100 p-2 no-rounded" min="1" max="255" placeholder="{{ translating('placeholder-whatsapp-number') }}" name='whatsapp'>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row mt-4">
            ...
            <div class="col-md-4 col-sm-6">
                <div class="row no-gutters">
                    <div class="col-xl-3 col-lg-5 col-sm-4 col-3">
                        @if(isset($countries) && count($countries) > 0)
                            <select style="-webkit-appearance: none;" form="changeUserPhonesForm" name="telegram_country_id" class="w-100 no-rounded input-default p-2" required>
                                @foreach($countries as $country)
                                    @if(isset($user['phone_number']['telegram_country_id']) && $user['phone_number']['telegram_country_id'] != NULL)
                                        <option value="{{ $country->id }}" @if($country->id == $user['phone_number']['telegram_country_id']) selected @endif>{{ '+ '.$country->code.' '.$country->{'title_'.app()->getLocale()} }}</option>
                                    @else
                                        <option value="{{ $country->id }}" @if($country->id == 121) selected @endif>{{ '+ '.$country->code.' '.$country->{'title_'.app()->getLocale()} }}</option>
                                    @endif
                                @endforeach
                    ...
                    <div class="col-xl-9 col-lg-7 col-sm-8 col-9">
                        <input value="@if(isset($user['phone_number']['telegram'])) {{ $user['phone_number']['telegram'] }} @endif" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" form="changeUserPhonesForm" type="text" class="input-default w-100 p-2 no-rounded" min="1" max="255" placeholder="{{ translating('placeholder-telegram-number') }}" name='telegram'>
                    </div>
                </div>
            </div>
        </div>
...
<div class="row no-gutters mt-2">
    <form id="changeUserPhonesForm" action="{{ route('account-change-phone-numbers', ['locale' => app()->getLocale()]) }}" method="post" class="w-100">
        @csrf
        <button id="sendForm" type="submit" form="changeUserPhonesForm" class="float-right btn pl-3 pr-3 btn-main text-light btn-lg">{{ translating('save-canges') }}</button>
        <p class="clearfix"></p>
    </form>
</div>