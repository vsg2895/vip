                    ...
                    <div class="row mb-5 item-card-row" data-item-id="{{ $post->id }}">
                        ...
                        <div class="col-lg-3 col-sm-4">
                            ...
                            @if(isset($post->img) && $post->img != NULL) <!-- Has Image -->
                                <img src="{{ asset('assets/img/items/placeholder/placeholder.gif') }}"
                                     data-src="{{ asset('assets/img/items/'.$post->img) }}"
                                     class="lazy w-100 responsive" title="{{ $post->title }}" alt="{{ $post->title }}">
                            @else <!-- Has not Image -->
                                <img src="{{ asset('assets/img/items/placeholder/placeholder.gif') }}"
                                     data-src="{{ asset('assets/img/items/placeholder/no-image.jpg') }}"
                                     class="lazy w-100 responsive" title="{{ $post->title }}" alt="{{ $post->title }}">
                                @endif
                            </a>
                        </div>
                            ...
                            <div class="row no-gutters bottom-row-with-like">
                                ...
                                @auth <!-- Add to wishlist -->
                                    <div class="like-action">
                                        <!-- Liked -->
                                        <svg class="wishlist-action-searches float-right"
                                             data-url="{{ route('searchs-wishlist-action', ['locale' => app()->getLocale(), 'post_id' => $post->id]) }}"
...
@else
    <!-- Not data exists -->
        <p>{!! translating('wished-searchs-not-data-found-description') !!}</p>
    @endif
</div>
