        ...
        <div class="tab-content" id="paymentsTabContent">
            ...
            @foreach($payment_methods as $payment_method)
                <div class="tab-pane fade @if($loop->first) show active @endif" id="payment-method-{{ $payment_method->id }}" role="tabpanel" aria-labelledby="payment-method-{{ $payment_method->id }}-tab">
                    ...
                    @if(isset($payment_method->option) && count($payment_method->option) > 0)
                        <div class="row">
                            ...
                            <div class="col-lg-2 col-md-3 col-sm-4 col-12">
                                <select class="input-default p-2 pl-3 pr-3 mt-2 mb-4 h5">
                                    <!-- Loop from options -->
                            ...
                            <div class="col-lg-6 col-md-6 col-sm-6 col-12">
                                <!-- Form -->
                                <form action="3" method="post" class="w-100">
                                    @csrf
                                    <!-- Submit -->
                                    <button type="submit" class="btn btn-main text-light btn-lg">{{ translating('pay-now') }}</button>
                                </form>
                            </div>
                        </div>
                    @endif
                    ...
                    <!-- Description -->
                    <p class="h5">{!! $payment_method->{'description_'.app()->getLocale()} !!}</p>
                </div>
            @endforeach
        </div>
    @endif
